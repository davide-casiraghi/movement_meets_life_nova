    ...
    @if(count($event->teachers))
        <div class="flex mt-2">
            ...
            @foreach ($event->teachers as $key => $teacher)
                <a class="textLink" href="{{route('teachers.show', $teacher->id)}}">{{$teacher->name}}</a>@if(!$loop->last),@endif
            @endforeach
        </div>
    @endif
    ...
    @if(count($event->organizers))
        <div class="flex mt-2">
            ...
            @foreach ($event->organizers as $key => $organizer)
                <a class="textLink" href="{{route('organizers.show', $organizer->id)}}">{{$organizer->name}}</a>@if(!$loop->last),@endif
            @endforeach
        </div>
    @endif
